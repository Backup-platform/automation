...
test.describe('Validate desktop "in page" profile menu navigation', () => {

	test(`Test navigation`, async ({ menuItems, profileMenu, profileMenuDesktop, page, cashierGeneral }) => {
		const profileMenuScenarios = [
			{ buttonGetter: profileMenuDesktop.myBonusesButtonElement, url: `myBonuses` },
			{ buttonGetter: profileMenuDesktop.personalInfoButtonElement, url: `personalInfo` },
			{ buttonGetter: profileMenuDesktop.verificationButtonElement, url: `verification` },
			{ buttonGetter: profileMenuDesktop.historyButtonElement, url: `history` },
			{ buttonGetter: profileMenuDesktop.limitsButtonElement, url: `responsibleGaming` },
		...
		await profileMenu.validateProfileMenuDialogNotVisible();
		await profileMenuDesktop.validateProfileMenuElements();
		for (const { buttonGetter, url } of profileMenuScenarios) {
			await profileMenuDesktop.validateNavigation(buttonGetter, `${process.env.URL}/profile?tab=${url}`);
			//TODO: validate Page Elements when POM is ready ( future work )
			await profileMenuDesktop.validateProfileMenuElements();
			await profileMenuDesktop.clickDepositButton();
			await cashierGeneral.validateMainMenuVisible();
			await cashierGeneral.clickCloseButton();
			await cashierGeneral.validateMainMenuNotVisible();
			await assertUrl(page, `${process.env.URL}/profile?tab=${url}`, true);
		}
		await profileMenuDesktop.clickLogoutButton();
		await assertUrl(page, `${process.env.URL}`, true);
		await menuItems.validateGuestItems();
	});
});

    test.describe('Test menu from other pages', () => {
