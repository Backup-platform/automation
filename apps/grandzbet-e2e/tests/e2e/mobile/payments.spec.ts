...
test.describe.skip('Validate payments', () => {
  ...
  test(`Cashier e2e scenario`, async ({
    cashierGeneral,
    menuItems,
    wallet,
  }) => {
    ...
    const expectedRealMoneyAfterDeposit = calculateExpectedBalance(
      initialRealMoney,
      depositAmount
    );
    const expectedTotalAfterDeposit = calculateExpectedBalance(
      initialTotalBalance,
      depositAmount
    );
    ...
    const expectedRealMoneyAfterWithdraw = calculateExpectedBalance(
      expectedRealMoneyAfterDeposit,
      withdrawAmount
    );
    const expectedTotalAfterWithdraw = calculateExpectedBalance(
      expectedTotalAfterDeposit,
      withdrawAmount
    );
