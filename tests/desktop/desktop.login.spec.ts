...
test.beforeEach(async ({ page, banner, headerMenuDesktop }) => {
  await page.goto("https://stage.spacefortuna1.com/en/en");
...
test.describe("Login Desktop", () => {
  ...
  test.use({ storageState: "playwright/.auth/noAuthentication.json" });
  test("Validate Wrong Password Login", async ({
    landingPage,
    logInIFrame,
  }) => {
    await landingPage.clickLoginButton();
  ...
  test.use({ storageState: "playwright/.auth/noAuthentication.json" });
  test("Validate Wrong Username Login", async ({
    landingPage,
    logInIFrame,
  }) => {
    await landingPage.clickLoginButton();
  ...
  test.use({ storageState: "playwright/.auth/noAuthentication.json" });
  test("Validate Empty Username tab", async ({
    landingPage,
    logInIFrame,
    banner,
  }) => {
  ...
  test.use({ storageState: "playwright/.auth/noAuthentication.json" });
  test("Validate Empty Password tab", async ({
    landingPage,
    logInIFrame,
    banner,
  }) => {
  ...
  test.describe("Reset Password", () => {
    test("Valid email for Reset password", async ({
      landingPage,
      logInIFrame,
    }) => {
      await landingPage.clickLoginButton();
    ...
    test("Invalid email for Reset password", async ({
      landingPage,
      logInIFrame,
      resetPasswordFrame,
    }) => {
    ...
    test("Empty email tab for Reset password", async ({
      landingPage,
      logInIFrame,
    }) => {
      await landingPage.clickLoginButton();
    ...
    test('Invalid email without "@" for Reset password', async ({
      landingPage,
      logInIFrame,
      resetPasswordFrame,
    }) => {
    ...
    test('Invalid email without text after "@" for Reset password', async ({
      landingPage,
      logInIFrame,
      resetPasswordFrame,
    }) => {
