...
import { step, stepParam, assertAttribute, assertUrl, clickElement, assertVisible, validateAttributes } from '@test-utils/navigation.po';
...
export class TopCategories {
    ...
    public validateCardIconVisible = async (nthCard: number) =>
    ...
    public async validateCardElements(softAssert = true): Promise<void> {
        ...
        for (let i = 0; i < cardCount; i++) {
            await test.step(`I check card number ${i} elements are visible`, async () => {
                ...
                await this.validateCardIconVisible(i);
            });
        }
    }

    @stepParam((newURL, nthElement) => `I click on the top card ${nthElement} and validate navigation to ${newURL}`)
    public async validateTopCardNavigation(newURL: string, nthElement: number): Promise<void> {
        await this.clickTopCard(nthElement);
